...
public class DbReadTableUtil {
	...
	public static List<String> readAllTableNames() throws SQLException {
		...
		try {
			...
			conn = DriverManager.getConnection(CodeConfigProperties.url, CodeConfigProperties.username, CodeConfigProperties.password);
			stmt = conn.createStatement(1005, 1007);
			if (CodeConfigProperties.databaseType.equals("mysql")) {
				sql = MessageFormat.format("select distinct table_name from information_schema.columns where table_schema = {0}", new Object[] { TableConvert.getV(CodeConfigProperties.databaseName) });
			}
	...
	public static List<ColumnVo> readTableColumn(String tableName) throws Exception {
		String sql = null;
		List<ColumnVo> columntList = new ArrayList<ColumnVo>();
		try {
			log.info(CodeConfigProperties.diverName);
			System.out.println(TableConvert.getV(tableName.toLowerCase()));
			System.out.println(TableConvert.getV(CodeConfigProperties.databaseName));
			Class.forName(CodeConfigProperties.diverName);
			conn = DriverManager.getConnection(CodeConfigProperties.url, CodeConfigProperties.username, CodeConfigProperties.password);
			stmt = conn.createStatement(1005, 1007);
			if (CodeConfigProperties.databaseType.equals("mysql")) {
				sql = MessageFormat.format("select column_name,data_type,column_comment,numeric_precision,numeric_scale,character_maximum_length,is_nullable nullable from information_schema.columns where table_name = {0} and table_schema = {1}", new Object[] { TableConvert.getV(tableName.toLowerCase()), TableConvert.getV(CodeConfigProperties.databaseName) });
			}
			if (CodeConfigProperties.databaseType.equals("oracle")) {
			...
			if (CodeConfigProperties.databaseType.equals("sqlserver")) {
				sql = MessageFormat.format("select distinct cast(a.name as varchar(50)) column_name,  cast(b.name as varchar(50)) data_type,  cast(e.value as varchar(200)) comment,  cast(ColumnProperty(a.object_id,a.Name,'''Precision''') as int) num_precision,  cast(ColumnProperty(a.object_id,a.Name,'''Scale''') as int) num_scale,  a.max_length,  (case when a.is_nullable=1 then '''y''' else '''n''' end) nullable,column_id   from sys.columns a left join sys.types b on a.user_type_id=b.user_type_id left join (select top 1 * from sys.objects where type = '''U''' and name ={0}  order by name) c on a.object_id=c.object_id left join sys.extended_properties e on e.major_id=c.object_id and e.minor_id=a.column_id and e.class=1 where c.name={0} order by a.column_id", new Object[] { TableConvert.getV(tableName.toLowerCase()) });
			}
			ResultSet rs = stmt.executeQuery(sql);
			rs.last();
			...
			if (n > 0) {
				...
				columnt.setFiledComment(StringUtils.isEmpty(rs.getString(3)) ? columnt.getFieldName() : rs.getString(3));
				...
				if ((!CodeConfigProperties.DB_TABLE_ID.equals(columnt.getFieldName())) && (!CodeStringUtils.isIn(columnt.getFieldDbName().toLowerCase(), ui_filter_fields))) {
				...
				while (rs.previous()) {
					...
					if ((!CodeConfigProperties.DB_TABLE_ID.equals(po.getFieldName())) && (!CodeStringUtils.isIn(po.getFieldDbName().toLowerCase(), ui_filter_fields))) {
		...
		List<ColumnVo> rsList = new ArrayList<ColumnVo>();
		for (int i = columntList.size() - 1; i >= 0; i--) {
			ColumnVo ch = (ColumnVo) columntList.get(i);
			rsList.add(ch);
		}
		return rsList;
	}
	...
	public static List<ColumnVo> readOriginalTableColumn(String tableName) throws Exception {
		...
		try {
			...
			if (n > 0) {
				...
				throw new Exception("该表不存在或者表中没有记录");
			}
			log.debug("读取表成功");
			try {
				if (stmt != null) {
	...
	public static boolean checkTableExist(String tableName) {
		...
		try {
			...
			stmt = conn.createStatement(1005, 1007);
			if (CodeConfigProperties.databaseType.equals("mysql")) {
				sql = "select column_name,data_type,column_comment,0,0 from information_schema.columns where table_name = '" + tableName.toUpperCase() + "' and table_schema = '" + CodeConfigProperties.databaseName + "'";
			}
			if (CodeConfigProperties.databaseType.equals("oracle")) {
	...
	private static String formatFieldCapital(String field) {
		String[] strs = field.split("_");
		field = "";
		int m = 0;
		for (int length = strs.length; m < length; m++) {
